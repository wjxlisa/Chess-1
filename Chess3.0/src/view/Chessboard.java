...
public class Chessboard extends JComponent implements Serializable {
    ...
    public void initiateEmptyChessboard() {
        for (int i = 0; i < chessComponents.length; i++) {
            for (int j = 0; j < chessComponents[i].length; j++) {
                EmptySlotComponent em=new EmptySlotComponent(new ChessboardPoint(i, j), calculatePoint(i, j), clickController, CHESS_SIZE);
    ...
    private void initRookOnBoard(int row, int col, ChessColor color) {
        ...
        chessComponent.setInitChess(chessComponent);
    }
